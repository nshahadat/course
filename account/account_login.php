...
if (isset($_POST['loginbtn'])) {
    ...
    $sql = "SELECT * FROM account WHERE email = '$u_signin_email' AND password = '$u_signin_pass'";
    ...
    if ($numrows == 0) {
        ...
        window.location='./account_feed.php';
        </script>";
    } else {
        session_start();
        $_SESSION['email'] = $u_signin_email;
        $_SESSION['username'] = $data['name'];
        echo "<script>window.location='./account_feed.php'</script>";
    }
}
